...
void Interrupt_Handler::request(Interrupts interrupt) {
    auto interrupt_request = read(m_memory_bank_controller, interrupt_request_address);
    set_bit(&interrupt_request, static_cast<int>(interrupt));
    write(m_memory_bank_controller, interrupt_request_address, interrupt_request);
}

void Interrupt_Handler::clear(Interrupts interrupt) {
    auto request_register = read(m_memory_bank_controller, interrupt_request_address);
    clear_bit(&request_register, static_cast<int>(interrupt));
    write(m_memory_bank_controller, interrupt_request_address, request_register);
}

bool Interrupt_Handler::should_exit_halt() {

    const auto interrupt_requests = read(m_memory_bank_controller, interrupt_request_address);
    const auto interrupts_enabled = read(m_memory_bank_controller, interrupt_enabled_address);
...
bool Interrupt_Handler::is_requested(Interrupts interrupt) const {
    const auto request_register = read(m_memory_bank_controller, interrupt_request_address);
...
bool Interrupt_Handler::is_enabled(Interrupts interrupt) const {
    const auto interrupt_enable_register = read(m_memory_bank_controller, interrupt_enabled_address);
...
void Interrupt_Handler::call(Cpu& cpu, uint8_t interrupt_vector) {
    ...
    write(m_memory_bank_controller, static_cast<uint16_t>(cpu.stack_ptr - 1), pc_high);
    write(m_memory_bank_controller, static_cast<uint16_t>(cpu.stack_ptr - 2), pc_low);
